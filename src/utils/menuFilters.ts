...
export const filterMenuByShift = (categories: Category[], activeShiftId: number | string) => {
  ...
  const filtered = categories
    .map((category) => {
      ...
      const isCategoryVisible =
        category.isGlobal ||
        (category.shiftIds &&
          category.shiftIds.length > 0 &&
          category.shiftIds.some((id) => String(id) === String(activeShiftId))) ||
        !category.shiftIds ||
        category.shiftIds.length === 0; // Si no tiene turnos definidos, se muestra
      ...
      const filteredDishes = category.dishes.filter((dish) => {
        return (
          dish.isGlobal ||
          (dish.shiftIds &&
            dish.shiftIds.length > 0 &&
            dish.shiftIds.some((id) => String(id) === String(activeShiftId))) ||
          !dish.shiftIds ||
          dish.shiftIds.length === 0
        );
      });
  ...
  if (filtered.length === 0 && categories.length > 0) {
    console.warn(
      '⚠️ El filtro de turnos ocultó todo el menú. Devolviendo menú completo como fallback.',
    );
    return categories;
  }
