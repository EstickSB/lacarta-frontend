...
const MENU_ITEMS: MenuItem[] = [
  {
    ...
    name: 'Lomo Saltado',
    description:
      'Trozos de lomo fino salteados al wok con cebolla, tomate, ají amarillo y salsa de soya. Acompañado de papas fritas crujientes y arroz blanco.',
    price: 45.0,
    image:
  ...
  },
  {
    ...
    description:
      'Pesca del día marinada en leche de tigre clásica, ají limo, culantro y cebolla roja. Servido con choclo y camote glaseado.',
    price: 38.0,
    image:
      'https://images.unsplash.com/photo-1562967677-4c07b789975b?auto=format&fit=crop&w=600&q=80',
    category: 'Entradas',
    tags: ['pescatarian'],
    allergens: ['fish'],
    spiceLevel: 2,
  ...
  },
  {
    ...
    description:
      'Finas láminas de atún bañadas en salsa de tamarindo y soya, con toques de aceite de sésamo y chalaquita.',
    price: 42.0,
    image:
  ...
  },
  {
    ...
    description:
      'Arroz criollo al wok con mixtura de mariscos, ají panca, vino blanco y queso parmesano.',
    price: 48.0,
    image:
  ...
  },
  {
    ...
    name: 'Causa Limeña',
    description:
      'Suave masa de papa amarilla prensada con ají amarillo y limón, rellena de pulpa de cangrejo y palta.',
    price: 28.0,
    image:
  ...
  },
  {
    ...
    description:
      'Clásico postre peruano a base de manjar blanco de yemas, coronado con merengue al oporto y canela.',
    price: 22.0,
    image:
        ...
        <>
          ...
          >
            <div className="sticky top-0 right-0 p-4 flex justify-end z-10 pointer-events-none">
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full bg-black/50 backdrop-blur-md flex items-center justify-center text-white pointer-events-auto border border-white/10"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="-mt-14">
              <SkeletonImage
                src={dish.image}
                alt={dish.name}
                className="w-full h-72 rounded-t-[2rem]"
              />
            </div>
            ...
            <div className="p-6 pb-32">
              ...
              <p className="text-gray-400 text-sm leading-relaxed mb-6">{dish.description}</p>

              {/* Allergens & Info */}
              <div className="flex flex-wrap gap-2 mb-8">
                {dish.allergens.includes('fish') && (
                  <div className="flex items-center gap-1 bg-white/5 px-3 py-1.5 rounded-lg text-xs text-gray-300 border border-white/5">
                    <UtensilsCrossed size={12} /> Pescado
                  </div>
                )}
                {dish.allergens.includes('soy') && (
                  <div className="flex items-center gap-1 bg-white/5 px-3 py-1.5 rounded-lg text-xs text-gray-300 border border-white/5">
                    <Leaf size={12} /> Soya
                  </div>
                )}
...
const DinerMenu: React.FC = () => {
  ...
  return (
    ...
    >
      {/* DYNAMIC HEADER */}
      <div
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${isScrolled ? 'bg-[#050505]/90 backdrop-blur-xl border-b border-white/5 shadow-lg' : 'bg-transparent'}`}
      >
        <div className="flex items-center justify-between px-4 pt-safe-top h-16">
          ...
          <div className="flex gap-2">
            <button className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white">
              <Search size={16} />
            </button>
            <button className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white">
              <Info size={16} />
      ...
      <div
        ...
        className="flex-1 overflow-y-auto custom-scrollbar pt-32 pb-24 px-4 scroll-smooth"
      >
        {/* SMART FILTERS */}
        <div className="flex gap-2 mb-8">
          ...
          >
            <Star size={12} className={filter === 'STAR' ? 'fill-[var(--accent)]' : ''} /> Platos
            Estrella
          </button>
          <button
                      ...
                      <div className="relative aspect-square">
                        ...
                        {item.stockStatus === 'SOLD_OUT' && (
                          <div className="absolute inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center">
                            <span className="bg-black/80 text-white px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border border-white/10">
                              Agotado
                            </span>
                          </div>
                        )}
