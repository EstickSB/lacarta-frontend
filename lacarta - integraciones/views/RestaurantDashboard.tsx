...
const INITIAL_BOOSTS: MarketingBoost[] = [
  {
    id: 'b1',
    title: 'Hyper Visibility',
    description: 'Top 1 en mapas y búsquedas.',
    cost: 5.0,
    active: true,
    timeLeft: '2h 15m',
    impact: 85,
  ...
  },
  {
    id: 'b2',
    title: 'Star Dish',
    description: 'Destaca un plato con video autoplay.',
    cost: 2.0,
    active: false,
    impact: 0,
    type: 'DISH',
  },
  {
    id: 'b3',
    title: 'Flash Promo',
    description: 'Push a usuarios a 500m.',
    cost: 8.5,
    active: false,
    impact: 0,
    type: 'PROMO',
  },
  {
    ...
    description: 'Email a clientes inactivos.',
    cost: 12.0,
    active: true,
    timeLeft: '4d',
    impact: 42,
...
const INITIAL_MENU: MenuItem[] = [
  {
    id: 'm1',
    name: 'Lomo Saltado',
    description: 'Clásico peruano al wok.',
    price: 45,
    image:
  ...
  },
  {
    ...
    description: 'Pesca del día con chicharrón de calamar.',
    price: 38,
    image:
      'https://images.unsplash.com/photo-1562967677-4c07b789975b?auto=format&fit=crop&w=400&q=80',
    category: 'Entradas',
    tags: ['pescatarian'],
    ...
    <aside className="hidden lg:flex w-20 hover:w-64 transition-all duration-300 h-screen bg-[#0A0A0A] border-r border-white/5 flex-col fixed z-50 group">
      <div className="h-20 flex items-center justify-center group-hover:justify-start group-hover:px-6 transition-all border-b border-white/5">
        <span className="font-serif text-xl text-white font-bold">
          <span className="text-powerred">L</span>c.
        </span>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-2 py-6">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {currentView === item.id && (
                <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-powerred"></div>
              )}
              ...
              {!access && (
                <Lock
                  size={14}
                  className="text-powerred/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex-shrink-0"
                />
              )}
            </button>
          );
        ...
        <div className="mt-8 px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          ...
          <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
            ...
            <button
              onClick={openWallet}
              className="w-6 h-6 rounded-full bg-powerred flex items-center justify-center text-white hover:scale-110 transition-transform"
            >
              <Plus size={12} />
            </button>
          </div>
              ...
              {(table.status === 'OCCUPIED_DIGITAL' || table.status === 'OCCUPIED_MANUAL') && (
                <div>
                  ...
                  {table.allergensAlert && table.allergensAlert.length > 0 && (
                    <div
                      className="absolute top-3 right-3 text-powerred animate-pulse"
                      title="Alergia en Mesa"
                    >
                      <ShieldAlert size={16} />
                    </div>
                  )}
          ...
          >
            ...
            {selectedTable.status === 'AVAILABLE' && (
              <div className="flex-1 flex flex-col justify-center items-center text-center">
                <Button
                  onClick={() => handleStatusChange(selectedTable.id, 'OCCUPIED_MANUAL')}
            ...
            {selectedTable.status === 'PAYMENT_REQUEST' && (
              <div className="flex-1 flex flex-col justify-center">
                ...
                <Button
                  ...
                  className="w-full bg-green-500 hover:bg-green-600 border-none text-black"
                >
                  CONFIRMAR
                </Button>
              </div>
            ...
            {(selectedTable.status === 'OCCUPIED_DIGITAL' ||
              selectedTable.status === 'OCCUPIED_MANUAL') && (
          ...
          <div className="bg-gradient-to-br from-powerred/20 to-black border border-powerred/30 rounded-2xl p-6 md:p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 max-w-md">
              ...
              <p className="text-sm text-gray-400 mb-6">
                Descubre qué platos miran más tus clientes, gestiona tu stock en tiempo real y
                aumenta tus ventas con analíticas avanzadas.
              </p>
              <Button
                onClick={() => setPlan('PRO')}
                className="shadow-[0_0_20px_rgba(230,0,38,0.4)]"
              >
                Mejorar Plan a PRO
              </Button>
            </div>
            ...
            <div className="space-y-4 relative">
              ...
              <div className="space-y-4 blur-sm select-none opacity-50">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between p-3 bg-black/50 rounded-xl border border-white/5"
                  >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
                <Lock
                  size={32}
                  className="text-powerred mb-4 drop-shadow-[0_0_10px_rgba(230,0,38,0.8)]"
                />
                <p className="text-white font-bold text-center mb-4 max-w-xs">
                  ¿Quieres saber qué plato es el favorito de tus clientes?
                </p>
                <Button
                  onClick={() => setPlan('PRO')}
                  variant="secondary"
          ...
          <div className="bg-[#111] border border-white/5 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {INITIAL_MENU.map((item, idx) => (
                <div
                  key={item.id}
                  className="flex items-center justify-between p-3 bg-black/50 rounded-xl border border-white/5 hover:border-white/20 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <span className="text-xl font-serif font-bold text-gray-600 w-4">
                      {idx + 1}
                    </span>
                    <img
                      src={item.image}
      ...
      <div className="flex flex-col xl:flex-row gap-8 items-start border-b border-white/5 pb-12">
        ...
        <div className="w-full xl:w-auto space-y-6">
          <div className="flex justify-between items-end">
            ...
            <Button
              onClick={openWallet}
              className="text-xs !py-2 h-10 shadow-lg shadow-powerred/20"
            >
          ...
          <div className="w-full md:w-96 h-56 bg-gradient-to-br from-[#1a1a1a] to-black border border-white/10 rounded-3xl p-8 flex flex-col justify-between relative overflow-hidden group shadow-2xl">
            ...
            <div className="flex justify-between items-start relative z-10">
              <span className="font-serif font-bold text-white text-xl italic">
                LaCarta <span className="text-powerred">Black</span>
              </span>
        ...
        <div className="flex-1 w-full bg-[#111] p-6 rounded-3xl border border-white/5 self-stretch">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-green-900/20 text-green-500 flex items-center justify-center border border-green-500/20">
            ...
            <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-red-900/20 text-powerred flex items-center justify-center border border-powerred/20">
                  <TrendingUp size={16} />
                </div>
                <div>
            ...
            <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors opacity-50">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-gray-800 text-gray-500 flex items-center justify-center border border-gray-700">
                  <Receipt size={16} />
                </div>
                <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {boosts.map((boost) => (
            <div
              key={boost.id}
              className={`p-6 rounded-[2rem] border flex flex-col justify-between min-h-[320px] transition-all relative overflow-hidden group ${boost.active ? 'bg-[#111] border-powerred shadow-[0_0_30px_rgba(230,0,38,0.15)]' : 'bg-[#111] border-white/5 hover:border-white/20'}`}
            >
              <div>
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center border border-white/5 ${boost.active ? 'bg-powerred text-white shadow-lg shadow-powerred/30' : 'bg-white/5 text-gray-400'}`}
                  >
                  ...
                  <div
                    ...
                    className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors ${boost.active ? 'bg-green-500' : 'bg-gray-800'}`}
                  >
                    <div
                      className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${boost.active ? 'translate-x-6' : 'translate-x-0'}`}
                    ></div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 mb-6">
                  <span className="text-xs font-mono text-gray-300 font-bold bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
                    ${boost.cost.toFixed(2)} / día
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 bg-black/20 -mx-6 -mb-6 p-6 border-t border-white/5">
                ...
                <div className="flex justify-between text-[10px] uppercase font-bold text-gray-500">
                  ...
                  <span className={boost.active ? 'text-white' : ''}>
                    {boost.active ? `${boost.impact}%` : '0%'}
                  </span>
                </div>
  ...
  <div className="group flex items-center gap-4 p-3 bg-[#111] border border-white/5 rounded-xl hover:border-white/20 transition-all">
    ...
    <div className="hidden md:flex gap-1">
      {item.allergens.slice(0, 3).map((a) => (
        <span
          key={a}
          className="w-5 h-5 rounded-full bg-red-900/20 border border-red-900/50 text-red-500 text-[8px] flex items-center justify-center uppercase"
        >
          {a[0]}
        </span>
      ))}
      {item.tags.slice(0, 2).map((t) => (
        <span
          key={t}
          className="w-5 h-5 rounded-full bg-green-900/20 border border-green-900/50 text-green-500 text-[8px] flex items-center justify-center uppercase"
        >
          {t[0]}
        </span>
      ))}
    ...
    <div className="flex items-center gap-3 border-l border-white/5 pl-4">
      {isProOrUltra ? (
        <div className="flex flex-col items-center cursor-pointer">
          <div
            className={`w-8 h-4 rounded-full p-0.5 transition-colors ${item.stockStatus === 'AVAILABLE' ? 'bg-green-500' : 'bg-gray-700'}`}
          >
            <div
              className={`w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${item.stockStatus === 'AVAILABLE' ? 'translate-x-4' : 'translate-x-0'}`}
            ></div>
          </div>
        ...
        <div
          ...
          title="Gestión de stock requiere plan PRO"
        >
            ...
            >
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#111]">
                <h3 className="font-serif text-xl text-white">
                  {editingDish.id ? 'Editar Plato' : 'Nuevo Plato'}
                </h3>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-black/30 border border-white/10 rounded-lg p-3 text-sm text-white focus:border-powerred outline-none"
                    />
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[...ALLERGEN_OPTIONS, ...PREFERENCE_OPTIONS].map((opt) => (
                      <div
                        key={opt.id}
                        className="flex items-center gap-2 p-2 rounded-lg border border-white/5 bg-white/5 hover:bg-white/10 cursor-pointer"
                      >
                ...
                <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div
                      className={`w-10 h-6 rounded-full p-1 cursor-pointer transition-colors bg-green-500`}
                    >
                      <div
                        className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform translate-x-4`}
                      ></div>
                    </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    defaultValue="La Mar Cebichería"
                    className="w-full bg-black/30 border border-white/10 rounded-lg p-3 text-sm text-white focus:border-powerred outline-none"
                  />
                </div>
                <div className="space-y-1">
        ...
        {activeTab === 'HOURS' && (
          <div className="space-y-3">
            {['Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab', 'Dom'].map((day) => (
              <div
                key={day}
                className="flex items-center justify-between p-3 bg-[#111] border border-white/5 rounded-xl"
              >
                <div className="flex items-center gap-4">
                ...
                <div
                  className={`w-8 h-4 rounded-full p-0.5 cursor-pointer ${day === 'Dom' ? 'bg-gray-700' : 'bg-green-500'}`}
                >
                  <div
                    className={`w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${day === 'Dom' ? 'translate-x-0' : 'translate-x-4'}`}
                  ></div>
                </div>
              </div>
            ))}
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/5 text-center">
              ...
              <div className="flex items-center justify-center gap-4">
                <button
                  ...
                  className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center hover:bg-white/10 text-white"
                >
                  <Minus />
                </button>
                <span className="text-4xl font-mono text-white font-bold w-20 text-center">
                  {tableCount}
                </span>
                <button
  ...
  return (
    <div className="flex flex-col border-b border-white/5 bg-[#0A0A0A] sticky top-0 z-30">
      <div className="h-20 flex items-center justify-between px-4 md:px-8">
        ...
        <div className="hidden lg:block">{/* Breadcrumbs or Title */}</div>
...
const RestaurantDashboard: React.FC = () => {
  ...
  const [walletBalance, setWalletBalance] = useState(120.0);
          ...
          >
            ...
            <div className="h-full overflow-y-auto w-full custom-scrollbar">
              <div className="max-w-7xl mx-auto w-full">
                ...
                {currentView === 'PULSE' && (
                  <PulseView currentPlan={currentPlan} setPlan={setCurrentPlan} />
                )}
                ...
                <div className="py-10 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="text-xs text-gray-500 font-mono">
                    ...
                    <span className="text-white font-bold">
                      ${(walletBalance + rechargeAmount).toFixed(2)}
                    </span>
                  </div>
                </div>
              )}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#0A0A0A]/95 backdrop-blur-xl border-t border-white/10 p-2 flex justify-around z-50 pb-safe">
        {['HALL', 'PULSE', 'MENU', 'MARKETING', 'CONFIG'].map((id) => (
          <button
            ...
            className={`p-4 ${currentView === id ? 'text-powerred' : 'text-gray-500'}`}
          >
